...
export interface LegacyShatterButtonProps {
  children: ReactNode;
  className?: string;
  shardCount?: number;
  shatterColor?: string;
...
export function Component({
  ...
  shatterColor = "#00ffff",
  onClick,
}: LegacyShatterButtonProps) {
  const [isShattered, setIsShattered] = useState(false);
  const [shards, setShards] = useState<LegacyShard[]>([]);

  const handleClick = useCallback(() => {
    if (isShattered) return;

    const newShards: LegacyShard[] = [];
      ...
      <AnimatePresence>
        {isShattered && (
          <motion.div
            ...
            style={{ border: `2px solid ${shatterColor}`, boxShadow: `0 0 30px ${shatterColor}` }}
          />
        )}
      </AnimatePresence>
    </div>
