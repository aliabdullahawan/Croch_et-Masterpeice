...
function ScrollGlobe({ sections, globeConfig = defaultGlobeConfig, className }: ScrollGlobeProps) {
  ...
  return (
    <div className={cn("relative w-full max-w-screen overflow-x-hidden min-h-screen bg-background text-foreground", className)}>
      ...
      <div
        ...
        style={{ transform: globeTransform, filter: `opacity(${activeSection === 3 ? 0.4 : 0.85})` }}
      >
      ...
      {sections.map((section, index) => (
        ...
        >
          <div className="w-full max-w-sm sm:max-w-lg md:max-w-2xl lg:max-w-4xl xl:max-w-5xl will-change-transform transition-all duration-700 opacity-100 translate-y-0">
            ...
            <div className={cn("text-muted-foreground/80 leading-relaxed mb-8 sm:mb-10 text-base sm:text-lg lg:text-xl font-light", section.align === "center" ? "max-w-full mx-auto text-center" : "max-w-full")}>
            ...
            {section.features && (
              <div className="grid gap-3 sm:gap-4 mb-8 sm:mb-10">
                {section.features.map((feature, featureIndex) => (
                  <div key={feature.title} className="group p-4 sm:p-5 lg:p-6 rounded-lg sm:rounded-xl border bg-card/50 backdrop-blur-sm hover:bg-card/80 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 hover:border-primary/20 hover:-translate-y-1" style={{ animationDelay: `${featureIndex * 0.1}s` }}>
              ...
              <div className={cn("flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4", section.align === "center" && "justify-center", section.align === "right" && "justify-end", (!section.align || section.align === "left") && "justify-start")}>
                {section.actions.map((action, actionIndex) => (
                  ...
                  >
                    ...
                    {action.variant === "primary" && <div className="absolute inset-0 rounded-lg sm:rounded-xl bg-gradient-to-r from-primary to-primary/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />}
                  </button>
                ))}
              </div>
            )}
  ...
  const demoSections = [
    {
      ...
      align: "left" as const,
      actions: [
        { label: "Begin Journey", variant: "primary" as const },
